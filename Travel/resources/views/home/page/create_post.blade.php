...
@section('content')
    <div class="container mx-auto p-10 mt-10 bg-white rounded-lg shadow-md max-w-6xl">
        ...
        <div class="flex items-center space-x-4 mb-4">
            @if ($user->profile->avatar && file_exists(public_path('img/profile/avatar/' . $user->profile->avatar)))
                {{-- Avatar Container --}}
                <img src="{{ asset('img/profile/avatar/' . $user->profile->avatar) }}" alt="User Avatar"
                    class="w-12 h-12 rounded-full">
            @else
                {{-- Avatar Container --}}
                <img src="{{ asset('img/profile/avatar.png') }}" alt="User Avatar" class="w-12 h-12 rounded-full">
            @endif
        ...
        <form action="{{ route('store.post') }}" method="POST" id="blogForm" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <input type="text"
                    ...
                    id="title" name="title" required placeholder="Enter the title of your post">
            </div>
            ...
            <div class="mb-4">
                ...
                <input type="file"
                    ...
                    id="featuredImage" name="image_url" accept="image/*">
            </div>
            ...
            <div class="mb-4">
                ...
                <textarea id="content" name="content"
                    class="mt-1 block w-full p-3 border border-gray-300 rounded-md focus:outline-none focus:ring focus:ring-blue-300"
                    rows="5" placeholder="Write your content here..."></textarea>
            </div>

            <button type="submit"
                class="w-full py-3 mt-4 bg-blue-600 text-white font-semibold rounded-md hover:bg-blue-700 transition duration-200">Publish
                Post</button>
        </form>
    </div>
@endsection
