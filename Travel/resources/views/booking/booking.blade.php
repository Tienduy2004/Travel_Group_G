                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <input type="tel" name="dienThoai_contact" id="dienThoai"
                                    class="w-full p-2 border rounded" placeholder="Nhập số điện thoại" pattern="[0-9]*"
                    ...
                    <div class="bg-white p-6 rounded-lg shadow-md">
                        ...
                        <div class="mb-4">
                            <form id="promotionForm" method="POST">
                                @csrf
                                <label for="program_code" class="text-sm text-gray-600">Nhập mã khuyến mãi:</label>
                            ...
                            <p id="promotionMessage" class="text-sm text-gray-600 mt-2"></p>
                        </div>
                        ...
                        <div class="border-t pt-4 mb-4">
                            <div class="flex justify-between font-semibold text-lg">
                                <span>Tổng tiền</span>
                                <span class="text-red-600" id="totalAmount">{{ number_format($total_price) }} ₫</span>
                            </div>
                        </div>
...
<script>
    ...
    document.getElementById('applyPromotion').addEventListener('click', function () {
        ...
        const totalAmountElement = document.getElementById('totalAmount'); // Tổng tiền hiển thị
        const originalPrice = parseFloat(totalAmountElement.textContent.replace(/\D/g, '')); // Chuyển từ dạng "1.000.000₫" thành số
        const promotionMessage = document.getElementById('promotionMessage');

        if (originalPrice <= 0) {
            promotionMessage.textContent = "Tổng tiền phải lớn hơn 0 để áp dụng khuyến mãi.";
        ...
        fetch('/apply-promotion', {
            ...
            body: JSON.stringify({ program_code: programCode, original_price: originalPrice })
        })
            .then(response => {
                if (!response.ok) {
                    throw response;
            ...
            .then(data => {
                ...
                totalAmountElement.textContent = `${new Intl.NumberFormat('vi-VN').format(data.discounted_price)} ₫`;

                // Cập nhật giá trị trong input ẩn
                document.getElementById('totalAmount_input').value = data.discounted_price;
            ...
            .catch(async error => {
                ...
                promotionMessage.textContent = errorData.message;
                promotionMessage.classList.remove('text-green-600');
                promotionMessage.classList.add('text-red-600');
            });
    });
