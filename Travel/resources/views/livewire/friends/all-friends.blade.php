                    ...
                    <div class="p-4">
                        <div class="flex items-center gap-4">
                            <img alt="{{ $friend->name }}'s avatar"
                                 ...
                                 src="{{ asset('/img/profile/avatar/' . $friend->profile->avatar) }}" />
                            <div>
                                <!-- Thẻ liên kết đến trang profile của bạn bè -->
                                <a href="{{ url('profile?id=' . $friend->id) }}" class="font-semibold text-lg text-gray-800 dark:text-white">
                                    {{ $friend->name }}
                                </a>
                                <!-- Thời gian bạn bè -->
                                <p class="text-sm text-gray-500 dark:text-gray-400">
                                    Bạn bè từ {{ $friend->pivot->updated_at->diffForHumans() }}
                                </p>
                            </div>
                        </div>
                    </div>
